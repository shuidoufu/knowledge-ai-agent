...
三端架构：
- **Web 端**：Vue 3（`frontend/`，登录/应用中心/个人知识助手/AI 超级智能体）
- **微信小程序端**：uni-app CLI + Vite（`miniprogram/`，分支 `knowledge-miniprogram`，对齐 Web 端核心功能：应用中心首页、知识聊天（流式+RAG 引用）、Manus 聊天、语音输入/播报、历史会话管理）
- **安卓 APP 端**：WebView 壳工程（`android/`，远程加载已部署 H5）
...
miniprogram/ → 微信小程序端（独立 uni-app CLI 工程，Vue3 + Vite，编译到微信小程序；src/pages 页面层、src/components 组件层、src/utils 逻辑层（config/request/api/auth/chat/markdown/tts/stt/image-loader）、src/static/icons SVG 图标集）
```
...
script\html-to-md.bat D:\网页收藏目录
# 或 script/html-to-md.sh /path/to/html-notes

# 原始 Markdown 笔记清洗（默认输出到 document/yuque-sync）
script\preprocess-docs.bat D:\笔记目录
...
# 微信小程序编译（uni-app，详见 docs/miniprogram.md）
cd miniprogram && npm install && npm run dev:mp-weixin
# 产物：miniprogram/dist/dev/mp-weixin（微信开发者工具导入该目录预览；真机调试需真实 AppID + 勾选"不校验合法域名"）
```
...
**改动、新增、修复任何功能时，必须保持原有功能正常；凡是改动可能涉及的其他功能，改完后必须逐一回归测试，确保不受影响。**

- **改动前**：梳理本次改动与其他功能的关联点——共用组件（props/事件参数形态/生命周期）、全局请求层（request.js 拦截器/流式封装）、公共工具函数（markdown 预处理/图片加载/语音模块）、事件链（mp-html imgtap/linktap 参数形态）、状态共享（storage/响应式 ref）、样式类名、页面跳转链路等
- **改动后**：除验证本次改动目标外，必须对**所有受影响的功能**做回归验证（如：改图片渲染 → 回归图片点击放大/链接点击/图片懒加载；改请求层 → 回归登录/聊天/历史/语音全链路；改事件参数解析 → 回归同一事件源的所有消费方）
- **最隐蔽的风险**："改了一个功能却悄悄影响另一个正常功能"——这类影响使用者不会想到、也最难察觉（典型：共用组件的事件参数从事件对象改为直传 attrs、全局请求层加了 401 跳转影响登录页、工具函数改了正则影响另一处消费方）。改动完成后要主动站在"其他功能的使用者"角度检查一遍
- 自测完成后，把"本次改动影响/回归了哪些功能"一并告知用户
...
- [ ] **已回归测试本次改动涉及的其他功能？**（改动前梳理关联点 → 改动后逐一验证受影响功能，确保原有功能不被悄悄破坏）
