...
- **知识库管理页遗留问题（2026/9/16 独立审查发现，存量）**：
  ...
  ③ ~~**`canReindex` 与注释不符**：注释写"未入库与失败状态可重新入库"，实现为 `!isRunning(doc)`，导致已完成文档也显示「重新入库」按钮。~~ **已于 2026/9/16 修复**：改为 `REINDEXABLE_STATUSES`（未入库 / 预处理失败 / 向量化失败）白名单判断，与设计意图（「重新入库」是失败恢复手段）一致；已完成文档不再显示该按钮，因此也无需二次确认（无向量可丢）。
